...
#Backupexisitng configuration files
cp -p /etc/modprobe.d/CIS.conf /etc/modprobe.d/CIS.conf-$DATE
cp -p /etc/audit/auditd.conf /etc/audit/auditd.conf_$DATE
cp -p /etc/security/limits.conf /etc/security/limits.conf_$DATE
cp -p /etc/issue.net /etc/issue.net_$DATE
cp -p /etc/ssh/sshd_config /etc/ssh/sshd_config_$DATE
cp -p /boot/grub2/grub.cfg /boot/grub2/grub.cfg_$DATE
cp -p /etc/default/grub /etc/default/grub_$DATE
cp -p /etc/security/pwquality.conf /etc/security/pwquality.conf_$DATE
cp -p /etc/pam.d/password-auth /etc/pam.d/password-auth_$DATE
cp -p /etc/pam.d/system-auth /etc/pam.d/system-auth_$DATE
cp -p /etc/pam.d/postlogin /etc/pam.d/postlogin_$DATE
cp -p /etc/login.defs   /etc/login.defs_$DATE
############################################################################################################################
###########################################################################################################################
...
#if apt list -a ntp >/dev/null; then
...
#if apt list -a chrony >/dev/null; then
...
egrep -q "^(\s*)PermitRootLogin\s+\S+(\s*#.*)?\s*$" /etc/ssh/sshd_config && sed -ri "s/^(\s*)PermitRootLogin\s+\S+(\s*#.*)?\s*$/\1PermitRootLogin yes\2/" /etc/ssh/sshd_config || echo "PermitRootLogin yes" >> /etc/ssh/sshd_config
policystatus=$?
if [ $policystatus -eq 0 ]; then
...
egrep -q "^(\s*)max_log_file_action\s*=\s*\S+(\s*#.*)?\s*$" /etc/audit/auditd.conf && sed -ri "s/^(\s*)max_log_file_action\s*=\s*\S+(\s*#.*)?\s*$/\1max_log_file_action = ROTATE\2/" /etc/audit/auditd.conf || echo "max_log_file_action = ROTATE" >> /etc/audit/auditd.conf
policystatus=$?
if [ $policystatus -eq 0 ]; then
  echo -e "${GREEN}Remediated:${NC} Ensure audit logs are automatically deleted"
else
  echo -e "${RED}UnableToRemediate:${NC} Ensure audit logs are automatically deleted"
fi
...
egrep "^-w /etc/selinux/ -p wa -k MAC-policy" /etc/audit/audit.rules || echo "-w /etc/selinux/ -p wa -k MAC-policy" >> /etc/audit/audit.rules
egrep "^-w /usr/share/selinux/ -p wa -k MAC-policy" /etc/audit/audit.rules || echo "-w /usr/share/selinux/ -p wa -k MAC-policy" >> /etc/audit/audit.rules
...
egrep "^-w /var/log/lastlog -p wa -k logins" /etc/audit/audit.rules || echo "-w /var/log/lastlog -p wa -k logins" >> /etc/audit/audit.rules
egrep "^-w /var/run/faillock/ -p wa -k logins" /etc/audit/audit.rules || echo "-w /var/run/faillock/ -p wa -k logins" >> /etc/audit/audit.rules
...
egrep "^-w /var/run/utmp -p wa -k session" /etc/audit/audit.rules || echo "-w /var/run/utmp -p wa -k session" >> /etc/audit/audit.rules
egrep "^-w /var/log/wtmp -p wa -k logins" /etc/audit/audit.rules || echo "-w /var/log/wtmp -p wa -k logins" >> /etc/audit/audit.rules
egrep "^-w /var/log/btmp -p wa -k logins" /etc/audit/audit.rules || echo "-w /var/log/btmp -p wa -k logins" >> /etc/audit/audit.rules
...
egrep "^-w /etc/sudoers -p wa -k scope" /etc/audit/audit.rules || echo "-w /etc/sudoers -p wa -k scope" >> /etc/audit/audit.rules
egrep "^-w /etc/sudoers.d/ -p wa -k scope" /etc/audit/audit.rules || echo "-w /etc/sudoers.d/ -p wa -k scope" >> /etc/audit/audit.rules
...
egrep "^-w /var/log/sudo.log -p wa -k actions" /etc/audit/audit.rules || echo "-w /var/log/sudo.log -p wa -k actions" >> /etc/audit/audit.rules
policystatus=$?
if [ $policystatus -eq 0 ]; then
...
egrep "^-e 2" /etc/audit/audit.rules || echo "-e 2" >> /etc/audit/audit.rules
policystatus=$?
if [ $policystatus -eq 0 ]; then
